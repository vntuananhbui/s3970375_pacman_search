    ...
    def grade(self, gradingModule, exceptionMap={}, bonusPic=False):
        ...
        for q in self.questions:
            ...
            if len(incompleted) > 0:
                ...
                print(f"""
*** NOTE: Make sure to complete Question {prereq} before working on Question {q},
...
""")
    ...
    def produceGradeScopeOutput(self):
        ...
        for name in self.questions:
            ...
            test_out['output'] = f"  Question {name} ({test_out['score']}/{test_out['max_score']}) {'X' if not is_correct else ''}"
            test_out['tags'] = []
    ...
    def produceOutput(self):
        with open('edx_response.html', 'w') as edxOutput:
            ...
            total_possible = sum(self.maxes.values())
            total_score = sum(self.points.values())
            checkOrX = '<span class="incorrect"/>' if total_score < total_possible else '<span class="correct"/>'
            header = f"""
                <h3>
